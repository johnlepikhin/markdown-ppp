...
pub(crate) struct InlineIndex {
    ...
    closers: [Vec<usize>; 6],
    /// Offset of `[` → offset of the matching `]`.
    brackets: HashMap<usize, usize>,
    /// Sorted offsets of every `]`.
    close_brackets: Vec<usize>,
    /// Offset of `(` → offset of the matching `)`, for link destinations.
    parens: HashMap<usize, usize>,
    /// Sorted offsets of every unescaped `"`, `'` and `)` (link title delimiters).
    title_ends: [Vec<usize>; 3],
    /// Memoized end offsets of link destinations; see [`Self::destination_len`].
    destination_ends: RefCell<HashMap<usize, usize>>,
}
...
impl InlineIndex {
    pub(crate) fn build(input: &str) -> Self {
        ...
        Self {
            ...
            parens,
            title_ends,
            destination_ends: RefCell::new(HashMap::new()),
        }
    }
    ...
    pub(crate) fn bracket_match(&self, at: &str) -> Option<usize> {
        let from = self.offset(at)?;
        let close = *self.brackets.get(&from)?;
    ...
    pub(crate) fn destination_len(&self, at: &str) -> usize {
        ...
        let end = loop {
            ...
            if let Some(&end) = self.destination_ends.borrow().get(&pos) {
                break end.min(limit);
            }
            visited.push(pos);
...
pub(crate) fn destination_len_slow(at: &str) -> usize {
    let mut pos = 0;
    while pos < at.len() {
        let c = at[pos..].chars().next().unwrap();
        match c {
            '\\' => pos += escape_len(&at[pos..]),
            '(' => match paren_group_len(&at[pos..]) {
                Some(len) => pos += len,
                None => break,
            },
            ')' => break,
