...
impl GitHubAlertType {
    ...
    pub(crate) fn css_classes(&self) -> &'static str {
        match self {
            GitHubAlertType::Note => "markdown-alert markdown-alert-note",
            GitHubAlertType::Tip => "markdown-alert markdown-alert-tip",
    ...
    /// Get the SVG icon for the alert type
...
impl<'a> ToDoc<'a> for GitHubAlert {
    fn to_doc(&self, state: &'a State<'a>) -> DocBuilder<'a, Arena<'a>, ()> {
        ...
        tag(
            state,
            "div",
            vec![("class".to_owned(), self.alert_type.css_classes().to_owned())],
